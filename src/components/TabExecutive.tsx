...
export const TabExecutive: React.FC<TabExecutiveProps> = ({ data, selectedBranch }) => {
  ...
  // Filter branch data excluding GRAND TOTAL
  let branchesData = summaryOmset.filter(s => s.branch !== 'GRAND TOTAL');

  if (selectedBranch !== 'ALL') {
    branchesData = branchesData.filter(s => s.branch === selectedBranch);
  }

  // Sort by totalAgustus descending for Top 5 Omset
  const sortedBranchesByOmset = [...branchesData].sort((a, b) => b.totalAgustus - a.totalAgustus);
  const topBranches = [...sortedBranchesByOmset].slice(0, 5);

  // Sort by % Target Achievement ascending for 5 Cabang Perlu Evaluasi
  const sortedBranchesByAch = [...branchesData].sort((a, b) => {
  ...
  // Prepare chart data (Sorted by totalAgustus)
  const chartData = sortedBranchesByOmset.map(b => ({
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
          <div>
            ...
            <p className="text-xs text-slate-400">
              Perbandingan pencapaian target dan realisasi omset per cabang (dalam Rupiah)
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-panel rounded-2xl p-5 border border-slate-800">
          <div className="flex items-center gap-2 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Cabang dengan kontribusi omset nominal terbesar bulan ini</p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            {topBranches.map((b, idx) => {
              ...
              return (
                <div key={b.branch} className="flex items-center justify-between p-3 rounded-xl bg-slate-900/60 border border-slate-800/80 hover:border-amber-500/30 transition-all">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs font-bold text-slate-200">{b.branch}</p>
        ...
        {/* Bottom 5 / Need Focus (Sorted by % Target Achieved Ascending) */}
        <div className="glass-panel rounded-2xl p-5 border border-slate-800">
          <div className="flex items-center gap-2 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Cabang dengan persentase pencapaian target (% Achieved) paling rendah</p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            {bottomAchBranches.map((b, idx) => {
              const ach = b.target > 0 ? (b.totalAgustus / b.target) * 100 : 0;
              return (
                <div key={b.branch} className="flex items-center justify-between p-3 rounded-xl bg-slate-900/60 border border-slate-800/80 hover:border-rose-500/30 transition-all">
                  <div className="flex items-center gap-3">
      ...
      <div className="glass-panel rounded-2xl p-5 border border-slate-800 overflow-hidden">
        <div className="flex items-center gap-2 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Perbandingan pergerakan omset mingguan Agustus vs Juli</p>
          </div>
        </div>
